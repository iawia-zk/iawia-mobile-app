...
const _getSecurely = async function <T>(
  ...
  try {
    ...
    assert(simpleCheck.success, 'Authentication failed');
...
async function restoreFromMnemonic(mnemonic: string) {
  ...
  assert(ethers.Mnemonic.isValidMnemonic(mnemonic), 'Invalid mnemonic');
...
async function loadOrCreateMnemonic() {
  ...
  if (storedMnemonic) {
    try {
      ...
      console.log('Error parsing stored mnemonic, old secret format was used', e);
...
interface IAuthContext {
  ...
  restoreAccountFromMnemonic: (mnemonic: string) => Promise<SignedPayload<boolean> | null>;
...
export const AuthProvider = ({
  ...
  const [_, setAuthenticatedTimeout] = useState<ReturnType<typeof setTimeout>>();
  const [isAuthenticated, setIsAuthenticated] = useState(false);
  const [isAuthenticatingPromise, setIsAuthenticatingPromise] = useState<Promise<{
    success: boolean;
    error?: string;
  }> | null>(null);
  ...
  const loginWithBiometrics = useCallback(async () => {
    ...
    setAuthenticatedTimeout((previousTimeout) => {
      ...
      return setTimeout(() => setIsAuthenticated(false), authenticationTimeoutinMs);
    });
