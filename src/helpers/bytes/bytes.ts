...
import { MAX_BYTES_IN_FIELD } from 'constants/signatures';
...
export function splitToWords(number: bigint, wordsize: number, numberElement: number) {
  ...
  assert(
    t == BigInt(0),
    `Number ${number} does not fit in ${(wordsize * numberElement).toString()} bits`
  );
...
export function num2Bits(n: number, inValue: bigint): bigint[] {
  ...
  for (let i = 0; i < n; i++) {
    ...
    assert(out[i] === BigInt(0) || out[i] === BigInt(1), 'Bit value is not binary.');

    lc1 += out[i] * e2;
    e2 = e2 << BigInt(1);
  }

  assert(lc1 === inValue, 'Reconstructed value does not match the input.');
