import { attributeToPosition } from 'constants/hash';
import { IawiaAppDisclosureConfig } from '../appType/appType';
...
export function getAttributeFromUnpackedReveal(unpackedReveal: string[], attribute: string) {
  const position = attributeToPosition[attribute as keyof typeof attributeToPosition];
  let attributeValue = '';
...
export function getOlderThanFromCircuitOutput(olderThan: string[]): number {
  const ageString = olderThan.map((code) => String.fromCharCode(parseInt(code, 10))).join('');
  const age = parseInt(ageString, 10);
  return isNaN(age) ? 0 : age;
}

export function formatAndUnpackReveal(revealedData_packed: string[]): string[] {
  const revealedData_packed_formatted: string[] = [
...
export function formatAndUnpackForbiddenCountriesList(
  ...
  const forbiddenCountriesList_packed_formatted: string[] = [
...
export function revealBitmapFromAttributes(disclosureOptions: IawiaAppDisclosureConfig): string[] {
