...
export function flexiblePoseidon(inputs: bigint[]): bigint {
  switch (inputs.length) {
    ...
    default:
      unreachable('Unsupported number of inputs');
  }
}
...
export function hash(
  ...
  switch (hashFunction) {
    ...
    default:
      ...
      hashResult = sha512(unsignedBytesArray); // Default to sha512
    // default:
    //   console.log('\x1b[31m%s\x1b[0m', `${hashFunction} not found in hash`); // Log in red
  ...
  unreachable(`Invalid format: ${format}`);
}
...
export function customHasher(pubKeyFormatted: string[]) {
  if (pubKeyFormatted.length < 16) {
    ...
    assert(rounds <= 16, 'Number of rounds is greater than 16');

    const roundsHash = new Array(rounds);
    for (let i = 0; i < rounds; i++) {
      roundsHash[i] = { inputs: new Array(16).fill(BigInt(0)) };
    }
    for (let i = 0; i < rounds; i++) {
      for (let j = 0; j < 16; j++) {
        if (i * 16 + j < pubKeyFormatted.length) {
          roundsHash[i].inputs[j] = BigInt(pubKeyFormatted[i * 16 + j]);
        }
      }
    }
    const finalHash = flexiblePoseidon(roundsHash.map((h) => poseidon16(h.inputs)));
