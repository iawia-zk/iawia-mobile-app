...
import { flexiblePoseidon } from '../hash/hash';
...
export function hashEndpointWithScope(endpoint: string, scope: string): string {
  ...
  assert(endpointChunks.length <= 16, 'Endpoint must be less than 496 characters');
...
export function stringToBigInt(str: string): bigint {
  ...
  assert(/^[\x00-\x7F]*$/.test(str), 'Input must contain only ASCII characters (0-127)');
