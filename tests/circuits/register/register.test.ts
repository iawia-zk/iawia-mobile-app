...
// TODO: remove the need for pulling circuit name
...
testSuite.forEach(
  ({
    ...
    describe(`Register - ${dgHashAlgo.toUpperCase()} ${eContentHashAlgo.toUpperCase()} ${hashFunction.toUpperCase()} ${sigAlg.toUpperCase()} ${domainParameter} ${keyLength}${
      saltLength ? ` Salt:${saltLength}` : ''
    }`, function () {
      this.timeout(0);
      let circuit: any;
      ...
      const passportData = genMockPassportData(
        dgHashAlgo,
        eContentHashAlgo,
        `${sigAlg}_${hashFunction}_${domainParameter}_${keyLength}${
          saltLength ? `_${saltLength}` : ''
