...
describe('DateIsLessChecker Circuit Test', function () {
  ...
  function generateRandomDate(yearStart: number, yearEnd: number) {
  ...
  describe('Unvalidity Tests', function () {
    unvalidExpiryDates.forEach((date, index) => {
      it(`unvalidity check for expiry date ${genDateStr(
        unvalidExpiryDates[index]
      )} and current date ${genDateStr(currentDates[index])}, expired since: ${getAgeFromDates(
        unvalidExpiryDates[index],
        currentDates[index]
      )}`, async function () {
        const inputs = {
  ...
  describe('Validity Tests', function () {
    validExpiryDates.forEach((date, index) => {
      it(`validity check for expiry date ${genDateStr(
        validExpiryDates[index]
      )} and current date ${genDateStr(currentDates[index])} valid until: ${getAgeFromDates(
        currentDates[index],
        validExpiryDates[index]
      )}`, async function () {
        const inputs = {
